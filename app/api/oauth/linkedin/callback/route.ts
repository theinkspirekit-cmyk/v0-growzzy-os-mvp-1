  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=linkedin&error=${encodeURIComponent(errorDescription || error)}`
      );
    }

    if (!code || !state) {
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=linkedin&error=${encodeURIComponent('Missing code or state')}`
      );
    }
    ...
    if (stateError || !oauthState) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=linkedin&error=${encodeURIComponent('Invalid state')}`
      );
    }
    ...
    if (new Date(oauthState.expires_at) < new Date()) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=linkedin&error=${encodeURIComponent('State expired')}`
      );
    }
    ...
    const redirectUri = `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/api/oauth/linkedin/callback`;
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=linkedin&success=true&connectionId=${connection.id}`
    );
  } catch (error: any) {
    console.error('[v0] LinkedIn OAuth callback error:', error);
    const errorMsg = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=linkedin&error=${encodeURIComponent(errorMsg)}`
    );
  }
}
