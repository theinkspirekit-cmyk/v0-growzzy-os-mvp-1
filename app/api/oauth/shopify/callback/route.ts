  ...
  try {
    ...
    if (errorParam) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?error=${errorParam}`
      );
    }
    ...
    const redirectUri = `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/api/oauth/shopify/callback`;
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=shopify&success=true`
    );
  } catch (error: any) {
    console.error('[v0] Shopify OAuth callback error:', error);
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?error=${encodeURIComponent(error.message)}`
    );
  }
}
