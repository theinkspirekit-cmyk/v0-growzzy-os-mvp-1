  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=meta&error=${encodeURIComponent(errorDescription || error)}`,
      )
    }

    if (!code || !state) {
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=meta&error=${encodeURIComponent('Missing code or state')}`,
      )
    }
    ...
    if (stateError || !oauthState) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=meta&error=${encodeURIComponent('Invalid state')}`,
      )
    }
    ...
    if (new Date(oauthState.expires_at) < new Date()) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=meta&error=${encodeURIComponent('State expired')}`,
      )
    }
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=meta&success=true&connectionId=${connection.id}`,
    )
  } catch (error: any) {
    console.error('[v0] Meta OAuth callback error:', error)
    const errorMsg = error instanceof Error ? error.message : 'Unknown error'
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'https://v0-growzzyos.vercel.app'}/connections?platform=meta&error=${encodeURIComponent(errorMsg)}`,
    )
  }
}
