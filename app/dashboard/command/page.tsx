...
import { Command as CommandIcon, Grid, Users, LayoutDashboard, Database, Activity } from "lucide-react"
        ...
        <DashboardLayout>
            <div className="flex flex-col h-[calc(100vh-100px)] items-center justify-center text-center space-y-8 bg-[#F8FAFC]">
                <div className="w-24 h-24 bg-white rounded-xl shadow-lg border border-[#E2E8F0] flex items-center justify-center relative overflow-hidden group hover:scale-105 transition-transform duration-500">
                    ...
                    <CommandIcon className="w-10 h-10 text-[#1F57F5] z-10" />
                ...
                <div className="w-full max-w-3xl grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[
                        { label: "Global Search", desc: "Index across all entities", icon: Database, action: () => router.push('/dashboard/campaigns') },
                        { label: "Bulk Actions", desc: "Multi-platform orchestration", icon: LayoutDashboard, action: () => router.push('/dashboard/automations') },
                        { label: "System Health", desc: "Real-time API monitoring", icon: Activity, action: () => router.push('/dashboard/analytics') },
                    ].map((feature, i) => (
                        <button
                            key={i}
                            onClick={feature.action}
                            className="bg-white p-6 rounded-lg border border-[#E2E8F0] shadow-sm flex flex-col items-start gap-3 hover:border-[#1F57F5]/30 hover:shadow-md transition-all group text-left w-full"
                        >
